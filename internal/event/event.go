...
func New(id, projectID, playerID, eventType string, occurredAt, receivedAt time.Time, properties map[string]any) (*Event, error) {
	...
	if projectID == "" { return nil, ErrInvalidProjectID }
	playerID = strings.TrimSpace(playerID)
	if playerID == "" { return nil, ErrInvalidPlayerID }
	eventType = strings.TrimSpace(eventType)
	if eventType == "" { return nil, ErrInvalidType }
	if occurredAt.IsZero() { return nil, ErrInvalidOccurredAt }
	if receivedAt.IsZero() { return nil, ErrInvalidReceivedAt }

	normalized, canonical, err := normalizeProperties(properties)
	if err != nil { return nil, err }

	return &Event{
		id:id, projectID:projectID, playerID:playerID, eventType:eventType,
...
func (e Event) Properties() map[string]any {
	...
	if err != nil { panic("event contains invalid normalized properties") }
	return normalized
}
...
func normalizeProperties(properties map[string]any) (map[string]any, []byte, error) {
	if properties == nil { properties = map[string]any{} }
	raw, err := json.Marshal(properties)
	if err != nil { return nil, nil, errors.Join(ErrInvalidProperties, err) }
	...
	if normalized == nil { normalized = map[string]any{} }
