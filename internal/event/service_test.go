...
func TestIngestPersistsEventAndMarksRetryAsDuplicate(t *testing.T) {
	...
	players := &fakePlayerRepository{
		players: map[string]*player.Player{"proj_1/player_1": target},
	}
...
func TestIngestRejectsCrossProjectPlayer(t *testing.T) {
	...
	service := NewService(
		&fakePlayerRepository{
			players: map[string]*player.Player{"proj_a/player_1": target},
		},
		&fakeEventRepository{},
	)
...
func TestIngestRejectsConflictingReuseOfEventIdentity(t *testing.T) {
	...
	command.Properties = map[string]any{"lesson_id": "lesson_6"}
...
func (r *failingPlayerRepository) Save(context.Context, *player.Player) error {
	return r.err
}
