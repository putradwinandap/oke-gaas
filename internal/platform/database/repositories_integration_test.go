...
func openIntegrationDatabase(t *testing.T) *gorm.DB {
	...
	require.NoError(t, db.Exec("DROP TABLE IF EXISTS events CASCADE").Error)
	require.NoError(t, db.Exec("DROP TABLE IF EXISTS players CASCADE").Error)
	require.NoError(t, db.Exec("DROP TABLE IF EXISTS projects CASCADE").Error)
